...
class FriendInvites extends StatefulWidget {
  UserModel user;
  FriendInvites({Key? key, required this.user}) : super(key: key);

  @override
  State<FriendInvites> createState() => _FriendInvitestState(user);
}
...
class _FriendInvitestState extends State<FriendInvites> {
  UserModel user;
  _FriendInvitestState(this.user);
  @override
  void initState() {
    getInvites(user.token, user.id.toString());
    _friendList.clear();
    isPressed = false;
    super.initState();
  }
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    trailing: IconButton(
                        onPressed: () {
                          acceptInvite(user.token, friendsId!, user.id);
                          setState(() {
                            isPressed = true;
                          });
                        },
  ...
  Future getInvites(String token, String userid) async {
    ...
    var response = await http.post(
        Uri.https("1ce6-46-196-74-101.eu.ngrok.io",
            "Api/Friendship/GetReceivedFriendships"),
        headers: {
  ...
  Future acceptInvite(String token, int requestedbyid, int requestedtoid) async {
